...
iterations = 1;
algo_array = ["vasarhelyi", "olfati_saber","vignesh"];
drone_array = ["fixed_wing","quadcopter"];
nb_agents_array = [2, 4, 8, 16];%, 32, 64, 128, 256, 512, 1024]; % nb_agents to evaluate  
ct = zeros(length(iterations),  length(nb_agents_array) ,length(algo_array), length(drone_array));
    ...
    for selected_algo = 1:length(algo_array)
        for selected_drone = 1:length(drone_array)
            for selected_nb_agents = 1:length(nb_agents_array)
                ...
                ct(selected_iter,selected_nb_agents,selected_algo, selected_drone) = example_ct(algo_array(selected_algo), ...
...
yticks([0 0.1 1 10]);
legend('vasarhelyi, fixed_wing','olfati-saber, fixed_wing',"vignesh, fixed_wing",...
    'vasarhelyi, fixed_wing','olfati-saber, fixed_wing',"vignesh, fixed_wing", ...
        'Location','northwest')
%% End of simulation
