function [vel_command, collisions] = compute_vel_vignesh(self, p_swarm, r_agent, dt)
    ...
    acc_vignesh_cmd= zeros(3,nb_agents); % Vignesh's accl_command for limiting 
    ...
    idx_to_bound = (v_norm > p_swarm.vignesh_vmax);
    acc_vignesh_cmd(:,idx_to_bound)=acc_vignesh_cmd(:,idx_to_bound)+ ...
        p_swarm.vignesh_KPmax*(p_swarm.vignesh_vmax-v_norm(:,idx_to_bound)).*...
        (vel_command(:,idx_to_bound)/(v_norm(:,idx_to_bound)+0.0001 ));
    idx_to_bound = (v_norm < p_swarm.vignesh_vmin);
    acc_vignesh_cmd(:,idx_to_bound)=acc_vignesh_cmd(:,idx_to_bound)+ ...
        p_swarm.vignesh_KPmin*(p_swarm.vignesh_vmin-v_norm(:,idx_to_bound)).*...
        (vel_command(:,idx_to_bound)/(v_norm(:,idx_to_bound) +0.0001));
    ...
    accel_cmd = acc_vignesh_cmd + accel_cmd;
    accel_cmd_norm = sqrt(sum(accel_cmd.^2, 1));
    idx_to_bound = (accel_cmd_norm > p_swarm.max_a);
    accel_cmd(:,idx_to_bound)=accel_cmd(:,idx_to_bound)*p_swarm.max_a./accel_cmd_norm(1,idx_to_bound);
    % accel_cmd_norm = sqrt(sum(accel_cmd.^2, 1));
    % v_norm = sqrt(sum((vel_command.^2), 1));
    
    vel_command = vel + accel_cmd*dt;
...
function a_arena = repulsion_cubic_arena(pos_drone, pos_walls, wall_width, const_rep)
    ...
    % This will be a guarantee to ensure that
    % this function is being called
    % nb_agents = self.nb_agents;
    % vel_command = zeros(3, nb_agents);
    % nb_agent_collisions = 0;    % Nb of collisions among agents
    % nb_obs_collisions = 0;      % Nb of collisions against obstacles
    % min_dist_obs = 20;          % Init minimum distance to obstacles
    % vel = self.get_vel_ned();
