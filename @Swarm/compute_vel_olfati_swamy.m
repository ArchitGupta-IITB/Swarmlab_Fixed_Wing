function [vel_command, collisions] = compute_vel_olfati_swamy(self, p_swarm, r_agent, dt)
    ...
    acc_swamy_effect = zeros(3, nb_agents); % Swamy Effect
    acc_capped = zeros(3,nb_agents);    % Capping of Accl
    ...
    for agent = 1:nb_agents
        ...
        if p_swarm.is_swamy_active
            vel_swamy_rel_vel= vel(:, neig_lis) - vel(:,agent);
            vel_swamy_rel_vel_dir= sign(dot(vel_swamy_rel_vel), (cross([0,0,1], vel(:,agent) )) );
            acc_swamy_effect(:,agent) = p_swarm.gamma_swamy*sum( tanh( dot(  ( vel_swamy_rel_vel ) ,  (vel(:,agent)/sqrt(sum(vel(:,agent).^2,"all"))) )  ), "all"  ).*(  vel_swamy_rel_vel_dir .* cross( [0,0,1], vel(:,agent)/sqrt(sum(vel(:,agent).^2,"all"))  )   )    ;
        end
        ...
        acc_command(:, agent) = acc_pm(:, agent) + acc_vm(:, agent) + ...
            +acc_wall(:, agent) + acc_obs(:, agent) + acc_swamy_effect(:,agent);

        % Integrate acceleration to get velocity
        
        
        acc_parallel=min( dot(vel(:,agent)/sqrt(sum(vel(:,agent).^2,"all")),...
            acc_command(:,agent)) , p_swarm.a_max_swamy)*...
            vel(:,agent)/sqrt(sum(vel(:,agent).^2,"all"))  ;
        acc_perpendicular = acc_command(:,agent) - ...
            dot(vel(:,agent)/sqrt(sum(vel(:,agent).^2,"all")),...
            acc_command(:,agent));
        acc_perpendicular = min(sqrt(sum(acc_perpendicular.^2,'all')), 
        acc_capped(:,agent) = acc_parallel;

        vel_command(:, agent) = vel(:, agent) + acc_capped(:,agent) * dt;
